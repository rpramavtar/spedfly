    ...
    <div class="row">
      <div class="col-12">
        <div class="d-flex align-items-center justify-content-between mt-4 mb-3">
          <div>
            <h1 class="h3 text-gray-800">{{ $customer->name }} - {{ __('ui.wallet') }}</h1>
            <p class="text-muted">{{ __('ui.manage_customer_records_segments_and_contact_lists') }}</p>
          </div>
          <div>
...
<div class="modal fade" id="topupWalletModal" tabindex="-1" aria-labelledby="topupWalletModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form action="{{ route('admin.customers.wallet.topup', $customer) }}" method="post">
        @csrf
        <div class="modal-body px-4 py-3">
          <p class="text-muted">{{ __('ui.adjusting_wallet_balance_for_customer') }} <strong>{{ $customer->name }}</strong></p>
          <div class="mb-3">
...
<div class="modal fade" id="thresholdWalletModal" tabindex="-1" aria-labelledby="thresholdWalletModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form action="{{ route('admin.customers.wallet.threshold', $customer) }}" method="post">
        @csrf
        <div class="modal-body px-4 py-3">
          <p class="text-muted">{{ __('ui.setting_minimum_threshold_for_customer') }} <strong>{{ $customer->name }}</strong></p>
          <div class="mb-3">
            ...
            <small class="text-muted">{{ __('ui.threshold_notification_help_text') }}</small>
          </div>
        </div>
        <div class="modal-footer px-4 py-3">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('ui.close') }}</button>
